...
class AllDosRsaExport implements
    FromCollection,
    WithHeadings,
    ShouldAutoSize,
    WithMapping
{
    ...
    public function collection()
    {
        $query = '';
        if ($this->dosId != 1244) {
            $query = "SELECT u.*, s.name AS storeName FROM users u, store_cities sc, store s WHERE u.user_type = 'rsa' AND u.store_id = s.id AND s.city_id = sc.id AND s.dos = '$this->dosId' AND u.deleted_at is NULL ";
        } elseif ($this->dosId == 1244) {
            $query =
                "SELECT u.id, u.username, u.address, u.emp_number, u.email, u.password, u.first_name, u.last_name, u.zip, u.phone, u.created_at, u.status, s.name AS storeName, s.address AS storeAddress, s.zip AS storeZip, sc.name AS cityName FROM users u, store_cities sc, store s WHERE u.user_type = 'rsa' AND u.store_id = s.id AND s.city_id = sc.id AND s.account = '9977700' AND u.deleted_at is NULL ";
        }

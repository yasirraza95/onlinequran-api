...
class SelectedDosCorporateRsaExport implements
    FromCollection,
    WithHeadings,
    ShouldAutoSize,
    WithMapping
{
    ...
    {
        $expIds = explode('+', $this->dosId);
        foreach ($expIds as $dosId) {
            $this->_id[] = $dosId;
        }
        ...
        $query =
            "SELECT u.*, s.name AS storeName FROM users u, store_cities sc, store s WHERE u.user_type = 'corporate' AND u.store_id = s.id AND s.city_id = sc.id AND u.deleted_at is NULL AND u.id in (" .
            $whereIn .
            ")";
